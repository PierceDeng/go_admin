...
func LoadAuthRouter(authGroup *gin.RouterGroup) {
	...
	authGroup.POST("/system/user", api.UserControl.AddUser)
	authGroup.DELETE("/system/user/:userId", api.UserControl.DeleteUserBatch)
	authGroup.PUT("/system/user/resetPwd", api.UserControl.ResetUserPwd)
	authGroup.POST("/system/user/export", api.UserControl.QueryUserListForExport)
	authGroup.POST("/system/user/importData", api.UserControl.ImportUserTable)
